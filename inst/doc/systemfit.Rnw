...
\Address{
   ...
   E-mail: \email{arne.henningsen@googlemail.com}\\
...
The \codeD{linear}{hypothesis} and
...
The \codeD{linear}{hypothesis} method for {systemfit} objects
...
the default \codeD{linear}{hypothesis} method of the \pkg{car} package
...
\codeD{linear}{hypothesis} of the \pkg{car} package \citep{r-car-1.2-1, fox02a}.
...
(see documentation of function \codeD{linear}{hypothesis}
...
the method \codeD{linear}{hypothesis} for the $F$ and Wald tests
...
<<systemfit_usage>>=
linear.hypothesis( fitsur, Rmat, qvec, test = "FT" )
...
The first argument of the \codeD{linear}{hypothesis} method for \code{systemfit} objects
...
The method \codeD{linear}{hypothesis} returns the same value of the $F$ statistic
